...
from vk_for_bot import name_vk_user
...
def start(message: Message):
    ...
    bot.send_message(message.chat.id, f'Привет, {message.from_user.first_name}, более подробную информацию обо мне  можешь узнать по команде /help ')
...
def query_handler(call):
    if call.data == 'get_info':
        bot.send_message(call.message.chat.id, 'Меня зовут Бот - Артем,  я умею добывать информацию о пользователях из вконтакта по их ID, также могу сказать тебе твой персональный ID в телеграм.')
...
def user_answer(message):
    if has_cyrillic(message.text) == True:
        bot.send_message(message.chat.id, 'Извини, твой введенный адрес содержит русские буквы, попробуй повторить попытку')
    else:
        answer = (message.chat.id, f'{message.text}')
        bot.send_message(message.chat.id, name_vk_user(str(answer[1])))
        users[message.from_user.id] = {'id': False}
...
def get_user_text(message):
    if message.text == 'Привет' or message.text == 'привет':
        ...
        bot.send_message(message.chat.id, 'Извини, я пока не знаю, что тебе ответить(, введи команду /help и познакомься со мной')
